...
st.title("🏗️ 스마트 건축안전 현장 조사 시스템 (실무형)")
st.write(
...
if floor_plan_file is not None:
  ...
  if coord is not None:
    ...
    with st.form(
      ...
      st.subheader("📝 손상 부위 및 상세 제원 입력")

      # 구역 및 부재 정보
      col1, col2 = st.columns(2)
      with col1:
        location = st.selectbox(
            "📍 조사 위치", ["계단실", "복도", "외벽", "실내 벽체", "천장", "기타"]
        )
        member = st.selectbox(
            "🧱 대상 부재", ["벽체", "천장", "기둥", "보", "바닥", "개구부"]
        )
      with col2:
      ...
      st.markdown("---")
      st.markdown("##### 📏 균열 폭·길이 및 물량 상세 세분화")
      ...
      with col4:
        crack_length = st.text_input(
            "균열 길이 (m)", value="-", placeholder="예: 1.5"
        )
      with col5:
        ea = st.number_input(
      ...
      with col6:
        severity = st.selectbox(
            "상태 등급/심각도", ["경미 (A/B급)", "보통 (C급)", "심각 (D/E급)"]
        )
    ...
    for i in range(0, len(photo_items), 2):
      ...
      for j in range(2):
        if i + j < len(photo_items):
          p_no, p_fl, p_lc, p_mb, p_w, p_l, p_img = photo_items[i + j]
          with cols[j]:
            st.image(p_img, use_container_width=True)
            st.caption(
                f"NO.{p_no} | {p_fl} {p_lc} {p_mb} (폭:{p_w}mm, 길이:{p_l}m)"
            )
  else:
